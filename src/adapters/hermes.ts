...
import { envForRoute, resolveBackendModel, resolveRoute, resolveRouteApiKeyForConfig, validateModelIdForRoute } from "../providers/registry.ts"
...
export function renderHermesConfig(route: ProviderRoute, model: string): string {
  ...
  if (route.kind === "openai-compatible") {
    ...
    // Throws when a configured apiKeyEnv resolves to nothing (the hermes
    // child inherits this process's env, so it could never resolve later);
    // a deliberate `apiKey: ""` (auth-free local endpoint) passes through.
    const apiKey = resolveRouteApiKeyForConfig(route, "hermes (managed)")
    return [
